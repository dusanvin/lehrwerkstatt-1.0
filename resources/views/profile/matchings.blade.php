        ...
        <div class="px-1 md:px-8 py-1 md:py-8 text-gray-700 w-screen sm:rounded-r-lg bg-gray-900">
            ...
            <div class="mx-auto rounded">
                ...
                <div class="mt-1 mb-6 text-sm text-gray-300 grid text-center sm:text-left block">
                    ...
                    <p class="block mb-4">Kurz nach der jeweiligen Matchingrunde (Termine siehe <a
                            href="https://www.uni-augsburg.de/de/forschung/einrichtungen/institute/zlbib/lehrwerkstatt/"
                            class="underline hover:text-white">Webseite</a>), teilen wir Ihnen hier Ihren Tandemvorschlag mit
                        - insofern ein Match gebildet werden konnte. Sie erhalten eine E-Mail-Benachrichtigung, sobald
                        dieser einsehbar ist.</p>
                    ...
                    <div class="text-gray-300 px-4 py-2 flex items-center rounded-full">
                        ...
                        <div class="pl-3">
                            ...
                            <p class="navigation-element text-sm">Bitte <strong>kontaktieren</strong> Sie dann Ihre*n
                                potentielle*n Tandempartner*in für ein Kennenlernen.</p>
                    ...
                    <div class="text-gray-300 px-4 py-2 flex items-center rounded-full">
                        ...
                        <div class="pl-3">
                            ...
                            <p class="navigation-element text-sm">Nach dem Kennenlernen bitten wir Sie, das Tandem per Klick
                                auf den entsprechenden Button zu <strong>bestätigen</strong> oder
                                <strong>abzulehnen</strong>.</p>

                        </div>

                    </div>

                    <p class="block my-4">Mit der Bestätigung sagen Sie der Zusammenarbeit in diesem Tandem im Rahmen der
                        <em>Lehr:werkstatt</em> verbindlich zu.</p>

                </div>

                <!-- Anzahl -->

                @if (!isset($user->unresolved_matching))
                    <p
                        class="px-6 py-3 bg-gray-700 text-left text-xs leading-4 font-medium text-gray-400 uppercase tracking-wider mt-4 rounded-md font-semibold">
                        ...
                        <table class="min-w-full mt-4 mb-2 mr-4 shadow-sm rounded-lg">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <th
                                        class="hidden sm:table-cell px-6 py-3 border-b border-gray-200 bg-gray-700 text-left text-xs leading-4 font-medium text-gray-400 uppercase tracking-wider font-bold hidden sm:table-cell ">
                                        Schulart</th>
                                    ...
                                    <th
                                        class="hidden sm:table-cell px-6 py-3 border-b border-gray-200 bg-gray-700 text-left text-xs leading-4 font-medium text-gray-400 uppercase tracking-wider rounded-tr-md font-bold">
                                    </th>
                            ...
                            <tr class="border-t border-gray-200 bg-gray-700 text-sm text-gray-400">
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap align-top sm:table-cell">
                                    ...
                                    <a class="flex text-white hover:text-gray-100">
                                        ...
                                        {{ $user->unresolved_matching->vorname }} {{ $user->unresolved_matching->nachname }}

                                    </a>

                                    <a href="mailto:{{ $user->unresolved_matching->email }}"
                                        class="leading-5 text-gray-400 hover:text-gray-100 break-words">{{ $user->unresolved_matching->email }}</a>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap align-top hidden sm:table-cell">
                                    ...
                                    <div class="leading-5 font-normal select-none p-1 w-12 rounded-sm">
                                        ...
                                        {{ $user->unresolved_matching->survey_data->schulart }}
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap align-top hidden sm:table-cell">
                                    ...
                                    <div class="leading-5 font-normal select-none p-1 w-12 rounded-sm">
                                        ...
                                        {{ $user->unresolved_matching->faecher_as_string }}
                                ...
                                @if (strcasecmp($user->role, 'Stud') == 0)
                                    <td class="px-6 py-4 whitespace-no-wrap align-top hidden sm:table-cell">
                                        ...
                                        <div class="leading-5 font-normal select-none p-1 w-12 rounded-sm w-full">
                                            ...
                                            {{ $user->unresolved_matching->survey_data->schulname }}<br>
                                            {{ $user->unresolved_matching->survey_data->strasse }}
                                            {{ $user->unresolved_matching->survey_data->hausnummer }}<br>
                                            {{ $user->unresolved_matching->survey_data->postleitzahl }}
                                            {{ $user->unresolved_matching->survey_data->ort }}<br>
                                            {{ $user->unresolved_matching->survey_data->landkreis }}
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap align-top">
                                    ...
                                    <div class="py-2 mx-auto rounded-md">
                                        ...
                                        <div class="flex">
                                            ...
                                            <a href="/messages/create/{{ $user->unresolved_matching->id }}"
                                    ...
                                    @if (
                                        (strcasecmp($user->role, 'lehr') == 0 && !isset($user->unresolved_matching->pivot->is_accepted_lehr)) ||
                                            (strcasecmp($user->role, 'stud') == 0 && !isset($user->unresolved_matching->pivot->is_accepted_stud)))
                                        <form action="{{ route('acceptMatching') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="role" value="{{ $user->role }}">
                                            <input type="hidden" name="lehrid"
                                                value="{{ $user->role == 'Lehr' ? $user->id : $user->unresolved_matching->id }}">
                                            <input type="hidden" name="studid"
                                                value="{{ $user->role == 'Stud' ? $user->id : $user->unresolved_matching->id }}">
                                        ...
                                        <dialog id="textModal"
                                            class="rounded fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                            <h3 class="text-base font-semibold leading-6 text-gray-900 mb-1"
                                                id="modal-title">Vorschlag ablehnen</h3>
                                            <p class="mb-4 text-sm text-gray-500">Bitte teilen Sie uns mit, weshalb Sie den
                                                Vorschlag ablehnen, um zukünftige Paarungen passgenauer zu gestalten.</p>
                                            ...
                                            <form action="{{ route('declineMatching') }}" method="POST">
                                                ...
                                                <button type="button" onclick="closeDialog()"
                                                    style="position:absolute; top:0; right:1%;">&times;</button>
                                    ...
                                    @else
                                        @if (strcasecmp($user->role, 'lehr') == 0)
                                            {{ $user->unresolved_matching->pivot->is_accepted_lehr == 1 ? 'Sie haben den Tandemvorschlag bestätigt.' : 'Sie haben den Tandemvorschlag abgelehnt.' }}
                                        @elseif (strcasecmp($user->role, 'stud') == 0)
                                            {{ $user->unresolved_matching->pivot->is_accepted_stud == 1 ? 'Sie haben den Tandemvorschlag bestätigt.' : 'Sie haben den Tandemvorschlag abgelehnt.' }}
                                        @endif
                                    @endif
